...
public class MatrixGraph extends AbstractGraph {
  ...
  public void insert(Edge edge) {
    ...
    if(edge.edgetype==1)
    setEdgeValue(edge.getSource(), edge.getDest(),edge.getTime());
    if(edge.edgetype==2)
    setEdgeValue(edge.getSource(), edge.getDest(),edge.getQuality());
  }
  ...
  /** Method to set the value of an edge
      ...
      @param tm - The time
   */
    private void setEdgeValue(int source, int dest, int tm) {
    if (isDirected() || source <= dest) {
      edges[source][dest] = tm;
    }
    else {
      edges[dest][source] = tm;
    }
  }
  ...
  /** Method to set the value of an edge
      @param source - The source vertix
      @param dest - The destination vertix
      @param qt - The quality
   */
    private void setEdgeValue(int source, int dest, float qt) {
    if (isDirected() || source <= dest) {
      edges[source][dest] = qt;
    }
    else {
      edges[dest][source] = qt;
    }
  }
